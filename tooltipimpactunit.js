...
{ const c=loadGame('./GrimGrind.html'); c.newGame('bow','keys'); c.setLanguage('ru');
  ...
  ok('панель создаётся внутри колонки сразу после своей карточки',
    fs.readFileSync('./GrimGrind.html','utf8').includes("'</div>' + levelUnlockPanelHtml([c], linkedUnlocks) + '</div>'")); }
...
{ const c=loadGame('./GrimGrind.html'); c.newGame('bow','keys'); c.setLanguage('ru');
  ...
  ok('наведение открывает подробную подсказку будущего навыка',
    ...
    fs.readFileSync('./GrimGrind.html','utf8').includes('el.onmouseenter = ev => showSkillTip(ev, preview, {note})'));
