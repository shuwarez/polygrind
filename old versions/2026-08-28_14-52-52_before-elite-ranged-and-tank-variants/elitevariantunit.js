/* Шесть разновидностей ближней элиты: листы, выбор, защита и контактные эффекты. */
const fs=require('fs');
...
{ const c=loadGame('./PolyGrind.html'); c.newGame('bow','keys'); const G=c.__api.G; G.floor=20;G.enemies.length=0;
  ...
  ok('случайная элита назначает виды только Бегунам и Ядрам',sample.every(e=>
    (e.typeKey==='runner'||e.typeKey==='blob')?!!e.eliteVariant:!e.eliteVariant));
  ok('случайная выборка достигает обеих новых групп',sample.some(e=>e.typeKey==='runner'&&e.eliteVariant)&&sample.some(e=>e.typeKey==='blob'&&e.eliteVariant)); }
