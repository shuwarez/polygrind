...
function mk(subclass, lvl=20){
  const c=loadGame('./index.html'); c.newGame('blade','keys',subclass);
...
function orbitHit({subclass=null,lvl=1,inc=0,more=1,element=0,crit=0,double=0,ignite=0}={}){
  const c=loadGame('./index.html',{random:()=>0}); c.newGame('blade','keys',subclass);
...
{
  const c=loadGame('./index.html'), s=c.__api.SUBCLASSES.blade;
...
{
  const c=loadGame('./index.html'), card=c.__api.MODS.find(x=>x.id==='shape.orbit');
  ...
  const radiusGame=loadGame('./index.html'); radiusGame.newGame('blade','keys');
...
{
  const c=loadGame('./index.html'), card=c.__api.MODS.find(x=>x.id==='trig.on_damaged');
...
{
  const c=loadGame('./index.html'), thorns=c.__api.MODS.find(x=>x.id==='dmg.thorns'),
...

{
  ...
  const ranged=loadGame('./index.html'); ranged.newGame('bow','keys');
