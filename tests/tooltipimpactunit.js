...
{ const c=loadGame('./index.html'); c.newGame('bow','keys'); c.setLanguage('ru');
  ...
  ok('панель создаётся внутри колонки сразу после своей карточки',
    require('./harness').loadInspectionSource('./index.html').includes("'</div>' + levelUnlockPanelHtml([c], linkedUnlocks) + '</div>'")); }
...
{ const c=loadGame('./index.html'); c.newGame('bow','keys'); c.setLanguage('ru');
  ...
  ok('наведение открывает подробную подсказку будущего навыка',
    ...
    require('./harness').loadInspectionSource('./index.html').includes('el.onmouseenter = ev => showSkillTip(ev, preview, {note})'));
