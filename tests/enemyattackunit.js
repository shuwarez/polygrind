...
ok('все четыре обычных типа переключаются на соответствующий attack-файл',normal.every((key,i)=>{
  const e={kind:'norm',typeKey:key,animT:0};c.startEnemyAttackVisual(e,0.4);
  return c.enemySpriteMeta(e).src===c.__api.ASSETS.enemyAttack[key];
}));
ok('все двенадцать элитных типов переключаются на свой attack-файл',elite.every((key,i)=>{
  const e={kind:'elite',eliteVariant:key,animT:0};c.startEnemyAttackVisual(e,0.4);
  return c.enemySpriteMeta(e).src===c.__api.ASSETS.eliteAttack[key];
}));
...
const live=loadGame('./index.html');live.newGame('bow','keys');
...
ok('реальный рывок Забытого стража запускает его attack-лист',
  guard.eliteDashT>0&&guard.attackVisual&&live.enemySpriteMeta(guard).src===live.__api.ASSETS.eliteAttack.forgottenGuard);
