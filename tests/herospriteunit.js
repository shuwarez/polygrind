...
const randomSoundSelections=[0,.26,.51,.999].map((random,index)=>{
  ...
  const game=loadGame('./index.html',{Audio:FakeMenuAudio,random:()=>random});
...
const routedGame=loadGame('./index.html',{Audio:FakeMenuAudio,random:()=>.51});
...
const warriorRouteGame=loadGame('./index.html',{Audio:FakeMenuAudio,random:()=>.999});
...
const mageRouteGame=loadGame('./index.html',{Audio:FakeMenuAudio,random:()=>.26});
...
const filterGame=loadGame('./index.html',{Audio:FakeMenuAudio});
...
const musicGame=loadGame('./index.html',{Audio:FakeMenuAudio,localStorage:musicStorage});
// Между UI-звуками теперь предварительно загружается level-up. Ищем экземпляры
// по внешнему runtime-пути, а не по хрупкому порядковому смещению в общем Audio-банке.
const runtimeAudio=musicGame.__api.AUDIO_ASSETS;
const menuAudio=FakeMenuAudio.instances.findLast(sound=>sound.src===runtimeAudio.menu);
const confirmAudio=FakeMenuAudio.instances.findLast(sound=>sound.src===runtimeAudio.confirm);
const hoverAudio=FakeMenuAudio.instances.findLast(sound=>sound.src===runtimeAudio.hover);
ok('музыка играет один раз, загружается заранее и повторяется после паузы 15 секунд',
  ...
  menuAudio.src===runtimeAudio.menu && menuAudio.src.endsWith('.ogg') &&
...
const mutedGame=loadGame('./index.html',{Audio:FakeMenuAudio,localStorage:musicStorage}), mutedAudio=FakeMenuAudio.instances.at(-1);
...
ok('подтверждение загружается заранее и дополнительно приглушено на 20%',
  ...
  confirmAudio.volume===0.4 && confirmAudio.plays===1 && confirmAudio.src===runtimeAudio.confirm);
...
ok('Hover UI дополнительно тише на 20% и не наслаивается внутри элемента',
  ...
  Math.abs(hoverAudio.volume-0.08)<1e-12 && hoverAudio.plays===1 && hoverAudio.src===runtimeAudio.hover &&
...
const subclassIds=Object.values(loadGame('./index.html').__api.SUBCLASSES).flat().map(s=>s.id);
...
ok('каждый экран класса получает ровно три правильные рамки подклассов',
  Object.entries({bow:['thief','hunter','dancer'],wand:['destroyer','multiplier','elementalist'],
    necro:['graverobber','animator','venomancer'],blade:['berserker','guardian','swordmaster']}).every(([wk,ids])=>{
      const game=loadGame('./index.html'); game.subclassScreen(wk);
...
{
  const c=loadGame('./index.html'); c.startScreen();
...
ok('описания классов короткие и не содержат внутренних имён параметров',
  Object.values(loadGame('./index.html').__api.WEAPONS).every(w=>w.desc.length<110 && !/wpn\.|min\.\*/.test(w.desc)));

function game(key){
  const c=loadGame('./index.html'); c.newGame(key,'keys');
