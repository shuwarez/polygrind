...
function runtimeAssetBuffer(reference,root='.'){
  if(typeof reference!=='string')return Buffer.alloc(0);
  const inline=reference.match(/^data:[^,]+;base64,(.+)$/);
  return inline?Buffer.from(inline[1],'base64'):fs.readFileSync(path.resolve(root,reference));
}
...
module.exports={imageInfo,embeddedImage,embeddedObjectImage,runtimeAssetBuffer};
