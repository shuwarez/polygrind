...
function runStaticCompatibility(){
  ...
  for (const weaponKey of WEAPONS){
    const c=loadGame('./PolyGrind.html',{random:rng});
...
function runCombatMatrix(){
  ...
  const catalog=loadGame('./PolyGrind.html',{random:rng});catalog.newGame('bow','keys',null,true);
  ...
  for(let i=0;i<specs.length;i++){
    ...
    if(!c){c=loadGame('./PolyGrind.html',{random:rng});contexts.set(key,c);}
...
try{
  const probe=loadGame('./PolyGrind.html',{random:makeRng(ROOT_SEED)});probe.newGame('bow','keys',null,true);
  report.build={htmlBytes:fs.statSync('./PolyGrind.html').size,bosses:probe.__api.BOSS_KEYS.length,
